...
class user(seat,name):
    ...
    def booking_details(self):
        ...
        if self.stadium_name == "Mumbai":
            temp_output =[[self.username, self.stadium_name, str(f"{mf.AWAY[self.match_booked-1]} X {mf.HOME[self.match_booked-1]} {mf.FORMAT[self.match_booked-1]}"), self.bookings, self.tot_cost, self.date, self.time]]
        if self.stadium_name == "Kolkata":
            temp_output =[[self.username, self.stadium_name, str(f"{kf.AWAY[self.match_booked-1]} X {kf.HOME[self.match_booked-1]} {kf.FORMAT[self.match_booked-1]}"), self.bookings, self.tot_cost, self.date, self.time]]
        if self.stadium_name == "Chennai":
            temp_output =[[self.username, self.stadium_name, str(f"{cf.AWAY[self.match_booked-1]} X {cf.HOME[self.match_booked-1]} {cf.FORMAT[self.match_booked-1]}"), self.bookings, self.tot_cost, self.date, self.time]]
        output=pd.DataFrame(temp_output)
        # Output file that shows booking confirmation details
        output.to_csv("booking_details.csv", index=False, mode="a", header=False)

    # Function to book tickets
    def book_ticket(self,cost,tot_capacity):
        self.bookings = int(input("Tickets to Book:"))
        self.date = datetime.date.today()
        t = datetime.time.localtime()
        self.time = datetime.time.strftime("%H:%M:%S", t)
